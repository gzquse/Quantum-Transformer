...
if __name__ == '__main__':
    ...
    # Probability distribution: [P(00), P(01), P(10), P(11)]
    dist = np.array([0.1, 0.2, 0.3, 0.4])
    marg = marginal_distribution(dist, [0])
    print(marg)
    
    # Apply CNOT with control=0 (leftmost qubit), target=1 (rightmost qubit)
    result = cnot_permutation(dist, control=0, target=1)
    print(result)

    print(get_bits(0b1101, [3, 2, 1, 0]))

    print(get_bit(4, 1))

    counts = {'00': 50, '01': 30, '10': 10, '11': 10}

    print(convert_shots_to_pdf(counts, normalize=True))
    
    yields = [
    {'00': 50, '01': 30, '10': 10, '11': 10},  # Experiment 1
    {'00': 25, '01': 25, '10': 25, '11': 25}   # Experiment 2
    ]
    nqubits = 2

    pdfs = yields_to_pdf(yields, nqubits, normalize=True)
    print(pdfs)
