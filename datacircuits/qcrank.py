...
class ReducedQCRANK(ParametrizedQCRANK):
    '''A reduced QCRANK circuit.'''
    def __init__(self, nq_addr, nq_data,
                 decoder: Type[_DecoderQCRANK] = QKAtan2DecoderQCRANK,
                 keep_last_cx: bool = True,
                 measure: bool = True,
                 ...
                 barrier: bool = True,
                 parallel: bool = True):
        super().__init__(nq_addr, nq_data, decoder, keep_last_cx, measure,
                         statevec, reverse_bits, barrier, parallel)
        # parameter vector
        self._p = [
            ParameterVector(f'p{i}', 2**nq_addr) for i in range(nq_data)
        ]
